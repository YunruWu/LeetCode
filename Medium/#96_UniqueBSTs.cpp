...
then the number of unique BST with the specified root F(i), is the cartesian product of the number of BST for its left and right subtrees. 
...
public:
    int numTrees(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            int result = 0;
        	for (int j = 1; j <= i; j++) {
        		result += results[j-1] * results[i-j];
        	}
